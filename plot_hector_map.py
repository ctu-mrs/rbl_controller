...
def plot_hector_map(bag_file_path, topic_name="/uav33/hector_mapping/map", target_time=None):
    ...
    # Plot the map
    fig, ax = plt.subplots()
    plt.imshow(map_data, cmap="gray_r", origin="lower", extent=extent)
    # plt.xlabel("X Position (meters)")
    # plt.ylabel("Y Position (meters)")
    # plt.colorbar(label="Occupancy Probability")


# Use the function with your ROS bag file and target time in seconds
plot_hector_map("_2024-10-24-17-56-24.bag", target_time= 1729785592.149783)  # 120 seconds into the bag
...
# Plot ID 1 data
plt.plot(id1_x, id1_y, color='red', linewidth=1, alpha=0.1)
plt.scatter(id1_x, id1_y, c=colors1, marker='o', s=100, alpha=0.1)

# Plot ID 3 data
plt.plot(id3_x, id3_y, color='blue', linewidth=1, alpha=0.1)
plt.scatter(id3_x, id3_y, c=colors3, marker='o', s=100, alpha=0.1)

# Plot ID 4 data
plt.plot(id4_x, id4_y, color='orange', linewidth=1, alpha=0.1)
plt.scatter(id4_x, id4_y, c=colors4, marker='o', s=100, alpha=0.1)

# Plot final data
plt.plot(final_x, final_y, color='green', linewidth=2, alpha=0.1)
plt.scatter(final_x, final_y, c=final_colors, marker='o', s=100, alpha=0.1)
...
plt.savefig("hectorplot.pdf")  # You can specify formats like .png, .jpg, .svg, .pdf, etc.
# Show plot
plt.show()
